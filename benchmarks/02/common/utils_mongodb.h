...
namespace media_service {
    ...
    init_mongodb_client_pool(const json &config_json, const std::string &service_name, uint32_t max_size) {
        auto mongodb = config_json["MediaService"]["mongodb"];
        if (mongodb.empty() ||
            !mongodb.contains("addr") ||
            !mongodb.contains("port")) {
            return nullptr;
        }
        std::string addr = mongodb["addr"];
        int port = mongodb["port"];
