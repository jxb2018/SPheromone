...
void run(Address public_ip, Address private_ip, unsigned thread_id) {
    ...
    while (true) {
        ...
        if (pollitems[0].revents & ZMQ_POLLIN) {
            ...
            notify_handler(serialized, pushers, bucket_key_address_map, bucket_triggers_map,
        ...
        if (pollitems[1].revents & ZMQ_POLLIN) {
            ...
            query_handler(serialized, pushers, bucket_key_address_map);
            query_count++;
        }
        ...
        if (pollitems[2].revents & ZMQ_POLLIN) {
            ...
            bucket_op_handler(serialized, pushers, bucket_triggers_map, app_buckets_map, bucket_app_map);
        }

        // handle trigger operation request
        if (pollitems[3].revents & ZMQ_POLLIN) {
            string serialized = kZmqUtil->recv_string(&trigger_op_handler_puller);
            trigger_op_handler(serialized, private_ip, thread_id, pushers, bucket_triggers_map, bucket_app_map,
                               node_status_map);
        }
        ...
        if (pollitems[6].revents & ZMQ_POLLIN) {
            ...
            app_register_handler(serialized, private_ip, thread_id, pushers, bucket_triggers_map, app_buckets_map,
        ...
        if (pollitems[7].revents & ZMQ_POLLIN) {
            ...
            func_call_handler(serialized, pushers, node_status_map);
            call_count++;
        }
