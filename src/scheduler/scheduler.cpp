...
void schedule_func_call(CommHelperInterface *helper, map<uint8_t, uint8_t> &executor_status_map,
 ...
 */
vector<string> check_trigger(BucketKey &bucket_key, string &session_id,
    ...
    check_object_arrival(bucket_key, bucket_triggers_map, active_triggers, active_func_metadata);
...
//                        log->info("Get local {}, recv: {}, ready: {}", key_name, recv_time, ready_time);
                ...
                if (infos.size() < 6) {
                    ...
                    if (str[1] == 2) {
                        ...
                        if (app_info_map[app_name].direct_deps_.find(src_function) !=
                            ...
                            if (tgt_function.empty()) {
                                ...
                                if (target_funcs.size() > avail_executors) {
                                    ...
                                    for (auto &func: local_funcs)
                                        schedule_func_call(helper, executor_status_map, function_executor_map,
                            ...
                            auto active_triggers = check_trigger(bucket_key, session_id, helper,
        ...
        if (!delay_call_queue.empty()) {
            ...
            while (cur_avail_executors > 0 && !delay_call_queue.empty()) {
                ...
                schedule_func_call(helper, executor_status_map, function_executor_map, delay_call.session_id_,
        ...
        if (duration >= schedTimerThreshold) {
            ...
            while (!rerun_check_queue.empty() && rerun_check_queue.top().check_time_ <= report_end) {
                ...
                for (auto &func_metadata: func_trigger_meta) {
                    schedule_func_call(helper, executor_status_map, function_executor_map, item.session_,
